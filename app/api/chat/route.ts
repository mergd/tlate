...
const tools = {
  ...
  searchWeb: tool({
    ...
    parameters: z.object({
      ...
      focus: z.enum(['academic', 'writing', 'math', 'programming', 'finance']).optional().describe('Search focus area')
    }),
        ...
        const response = await fetch('https://api.perplexity.ai/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: focus ? `You are a helpful research assistant. Focus on ${focus} topics.` : 'You are a helpful research assistant.'
              },
              {
                role: 'user',
                content: query
  ...
  translateText: tool({
    ...
    parameters: z.object({
      ...
      style: z.enum(['formal', 'informal', 'technical', 'creative']).optional().describe('Translation style')
    }),
...
function getModelProvider(modelId: string) {
  ...
  if (!model) return openai(DEFAULT_MODEL);
...
export async function POST(req: Request) {
  const { messages, context, model: selectedModel = DEFAULT_MODEL } = await req.json();
...
${context?.dictionary?.length > 0 ? `Project Dictionary:
